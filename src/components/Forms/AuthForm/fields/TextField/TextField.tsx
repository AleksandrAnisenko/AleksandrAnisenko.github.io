...
type TextFieldType = Extract<HTMLInputTypeAttribute, 'text' | 'password' | 'email' | 'number' | 'tel' | 'url'>;
      ...
      return (
        <div className={cn(classNames?.wrapper, s.outer)}>
          <div className={cn(s.fieldWrapper, { [s.fieldWrapper_error]: errorMessage })}>
            {label && (
              <label className={cn(s.label, { [s.label_top]: isFocused || value || defaultValue })} htmlFor={inputId}>
                {label}
              </label>
            )}
            <input
